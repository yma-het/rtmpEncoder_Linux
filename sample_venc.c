...
void *SAMPLE_VENC_1080P_CLASSIC(HI_VOID *arg)
{
    ...
    PIC_SIZE_E enSize[3] = {PIC_HD1080 ,PIC_HD720 ,PIC_QVGA};
	HI_U32 u32Profile = 0;
    ...
    ******************************************/
	...
	s32ChnNum = 1;
#endif
    s32ChnNum=1; // Do we actually need this dirty hack for gc1024?
	printf("s32ChnNum = %d\n",s32ChnNum);
    ...
    ******************************************/
	...
	if(s32ChnNum >= 1)
	{
	    ...
	    stVpssGrpAttr.enPixFmt = PIXEL_FORMAT_YUV_SEMIPLANAR_420; // Different format. So far considering that newest is corrcrt one
	    ...
	    stVpssChnMode.enPixelFormat  = PIXEL_FORMAT_YUV_SEMIPLANAR_420; // Different format. So far considering that newest is corrcrt one
...
HI_S32 SAMPLE_VENC_1080P_MJPEG_JPEG(HI_VOID)
{
    ...
    ******************************************/
    ...
    stVbConf.astCommPool[0].u32BlkSize = u32BlkSize;
    stVbConf.astCommPool[0].u32BlkCnt = g_u32RTMPBlkCnt;
...
// Here must be some function called SAMPLE_VENC_H264_IntraRefresh.
// And implementation of RTMP push mechanism has modifications in it.
// Probably those modifications are intended to change frequency of I-frames.
// But this SDK does not have it. This fact may cause some kind of problems.
